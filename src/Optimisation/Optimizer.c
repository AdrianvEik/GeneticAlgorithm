...
static void check_convergence(task_param_t* task, adaptive_memory_t* adaptive_memory, double best_result, gene_pool_t* gene_pool) {
    /*adaptive_memory->iteration_counter++;*/
    if (gene_pool->iteration_number > task->config_ga.optimizer_param.max_iterations) {
...
static void compute_mutation_rate(task_param_t* task, adaptive_memory_t* adaptive_memory, double best_result, gene_pool_t* gene_pool) {
...
void adapt_param(task_param_t* task, gene_pool_t* gene_pool, adaptive_memory_t* adaptive_memory) {
	...
	if (gene_pool->iteration_number == 0) {
    ...
    compute_mutation_rate(task, adaptive_memory, best_result, gene_pool);

    // Check for convergence & runtime params
    check_convergence(task, adaptive_memory, best_result, gene_pool);
