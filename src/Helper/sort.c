...
void recursive_indexed_merge_sort(double* arr, uint32_t* indices, uint32_t* temp_workspace, uint32_t lo, uint32_t hi, uint32_t size) {
	...
	recursive_indexed_merge_sort(arr, indices, temp_workspace, lo, mid, size);
	recursive_indexed_merge_sort(arr, indices, temp_workspace, mid, hi, size);
...
void indexed_merge_sort(double* arr, uint32_t* indices, uint32_t* temp_workspace, uint32_t size) {
	recursive_indexed_merge_sort(arr, indices, temp_workspace, 0, size, size);
}
...
static inline void bitonic_minmax_2v(__m512d* a, __m512d* b, __m256i* index_a, __m256i* index_b) {
	...
	__m512d swap_b = _mm512_permutexvar_pd(_mm512_set_epi64(7, 6, 5, 4, 3, 2, 1, 0), *b);// 0, 1, 2, 3, 4, 5, 6, 7
	__m256i swap_index_b = _mm256_permutexvar_epi32(_mm256_set_epi32(7, 6, 5, 4, 3, 2, 1, 0), *index_b); // 0, 1, 2, 3, 4, 5, 6, 7
...
static inline void post_sort_2v(__m512d* a, __m256i* index_a, __m512d* b, __m256i* index_b) {
	...
	*a = _mm512_mask_mov_pd(*a, cmp_res, *b);
...
void inline post_bitonic_merge(double* arr, uint32_t* indices, uint32_t* temp_workspace, uint32_t size, uint32_t intial_step_size) {
	uint32_t left, right, out, lo, mid, hi = 0;
    uint32_t step_size = intial_step_size;
	for (step_size = 8; step_size < size; step_size *= 2) {
		for (lo = 0; lo < size - step_size; lo += step_size * 2) {
			mid = lo + step_size;
			hi = (lo + step_size * 2 < size) ? (lo + step_size * 2) : size;
			left = lo;
			right = mid;
			out = lo;
			while (left < mid && right < hi) {
				if (arr[indices[left]] <= arr[indices[right]]) {
					temp_workspace[out++] = indices[left++];
				}
				else {
					temp_workspace[out++] = indices[right++];
				}
			}
			while (left < mid) {
				temp_workspace[out++] = indices[left++];
...
void indexed_bitonic_sort_1v(double* arr, uint32_t* indices, uint32_t* temp_workspace, uint32_t size) {
    ...
    post_bitonic_merge(arr, indices, temp_workspace, size, 8);
}
...
void indexed_bitonic_sort_2v(double* arr, uint32_t* indices, uint32_t* temp_workspace, uint32_t size) {
	...
	post_bitonic_merge(arr, indices, temp_workspace, size, 16);
}
...
void indexed_bitonic_sort_4v(double* arr, uint32_t* indices, uint32_t* temp_workspace, uint32_t size) {
    ...
    post_bitonic_merge(arr, indices, temp_workspace, size, 32);
}
...
void indexed_bitonic_sort_8v(double* arr, uint32_t* indices, uint32_t* temp_workspace, uint32_t size) {
    ...
    post_bitonic_merge(arr, indices, temp_workspace, size, 64);
}
...
void indexed_bitonic_sort_16v(double* arr, uint32_t* indices, uint32_t* temp_workspace, uint32_t size) {
    ...
    post_bitonic_merge(arr, indices, temp_workspace, size, 128);
}